...
const CENTER = [70.7957320, 22.3675808]; // [lng, lat] - Marwadi University
...
async function seed() {
  ...
  for (const eq of seedEquipment) {
    ...
    const photoUrl = eq.photo ? `/images/equipment/${eq.photo}` : null;
  ...
  for (const lab of seedLabour) {
    ...
    const photoUrl = lab.photo ? `/images/workers/${lab.photo}` : null;
